...
final class CentralManagerImpl
  ...
  Future<Peripheral> getPeripheral(String address) async {
    // SpotLink fork: build a peripheral handle from a persisted identifier.
    // The native side resolves unknown UUIDs through the system cache
    // (retrievePeripherals(withIdentifiers:)) at connect time, so known
    // peers can be reconnected across app relaunches without a scan.
    return PeripheralImpl(uuid: UUID.fromString(address));
  }

...
class CentralManagerImpl: CentralManagerHostApi {
    ...
    // SpotLink fork: opt in to state restoration so iOS relaunches the app
    // (background) when a pending connection completes after the process was
    // killed by the system. The delegate must be attached at creation or the
    // willRestoreState callback is lost.
    private lazy var mCentralManager = CBCentralManager(
        delegate: self.mCentralManagerDelegate,
        queue: nil,
        options: [CBCentralManagerOptionRestoreIdentifierKey: "spotlink.central"])
    ...
    // SpotLink fork: adopt peripherals restored by iOS (pending connects and
    // live connections survive the process) so later Dart calls can use them.
    func didRestoreState(peripherals: [CBPeripheral]) {
        for peripheral in peripherals {
            let uuidArgs = peripheral.identifier.uuidString.lowercased()
            peripheral.delegate = self.mPeripheralDelegate
    ...
    private func retrievePeripheral(uuidArgs: String) throws -> CBPeripheral {
        if let peripheral = self.mPeripherals[uuidArgs] { return peripheral }
        // SpotLink fork: fall back to the system cache so a persisted
        // identifier can be reconnected after an app relaunch without a scan.
        guard let uuid = UUID(uuidString: uuidArgs),
              let peripheral = self.mCentralManager.retrievePeripherals(withIdentifiers: [uuid]).first
        else { throw BluetoothLowEnergyError.illegalArgument }
        peripheral.delegate = self.mPeripheralDelegate
        self.mPeripherals[uuidArgs] = peripheral
        return peripheral
    }

...
- **DTN식 "언젠가 전달"**: 텍스트(+ACK)는 SQLite에 **무기한 영구 보관**되어 앱을
  재시작해도 다른 기기를 만날 때마다 계속 전파됨 (TTL 64홉). 파일은 24시간/512개로
  제한(남의 폰을 대용량으로 채우지 않음). 내 정보 탭 **"중계 보관함"**에서 보관량
  확인·비우기 가능
- **서명된 전달 영수증(전파 삭제)**: 수신자가 메시지를 받으면 **Ed25519 서명된
  RECEIPT를 플러딩** → 사본을 든 모든 중계 기기가 검증 후 삭제 + 묘비(tombstone)로
  좀비 재유입 차단. 서명 검증으로 제3자의 위조 삭제(검열) 불가. 영수증 자체도
  영구 보관되어 오래 꺼져 있던 기기도 나중에 정리됨
- **키 미보유 파킹**: 발신자 ANNOUNCE(3홉)보다 멀리 전달된 메시지는 복호화 키가
  없어도 버리지 않고 보관, 발신자의 ANNOUNCE가 도착하는 순간 자동 복호화·전달
...
- **Android 헤드리스 메시**: 재부팅·앱 강제종료·OEM 배터리 킬 후에도 서비스가
  자동 재기동되어 **화면 없이(TaskHandler 아이솔레이트) 메시 노드 전체를 구동** —
  수신·알림·릴레이·store-and-forward가 UI 없이 계속됨. 앱을 열면 핑/테이크오버
  프로토콜로 UI 메시에 무충돌 인계. 내 정보 탭에서 **배터리 최적화 예외** 요청
  (삼성 등 OEM 킬러 대응). iOS는 정책상 불가 → 백그라운드 유지 안내 표시
...
- **RSSI 근접 레이더**: 광고 수신·연결 링크 폴링(5초)의 신호 세기를 EMA로
  평활화해 사람 탭 상단 **소나 뷰**에 표시 — 나를 중심으로 가까운 사람일수록
  안쪽 링, 멀티홉은 최외곽 점선 링. 아바타 탭으로 바로 대화. 목록에도
  "바로 옆/가까이/근처/멀리" 라벨

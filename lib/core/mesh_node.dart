...
class MeshNode {
  ...
  static const Duration _transferTimeout = Duration(seconds: 45);
  ...
  Future<String?> sendFile(
    ...
    _senderTimers[tid] = Timer(_transferTimeout, () {
    ...
    // 2. Send all chunks (first pass; gaps are recovered via file ACK).
    var sent = 0;
    for (final chunk in sender.allChunks()) {
      ...
      sent++;
      _events.add(FileProgress(
          sender.meta.transferIdHex, sent / sender.meta.totalChunks, true));
    }
    bleLogSink?.call('FT send burst done: $name ($sent chunks)');
    return sender.meta.transferIdHex;
  }

  /// Route a locally originated frame: send to neighbours and keep a copy for
  /// store-and-forward so peers we meet later still receive it.
  ...
  Future<void> _handleFileChunk(PeerId from, Uint8List payload) async {
    ...
    } else if (isNew && receiver.receivedCount % 16 == 0) {
      // Periodic partial ACK so the sender can fill gaps early.
      await _sendFileAck(from, receiver.buildAck());
    }
  }
  ...
  /// While a transfer is incomplete, periodically re-ACK the missing seqs so a
  /// lost tail (chunks that never trigger the every-16 ACK) can't stall it.
  /// Gives up after [_transferTimeout] with no completion.
  void _startReceiverRecovery(String tidHex, PeerId from) {
    final deadline = DateTime.now().add(_transferTimeout);
